...
def get_chain_codes(x, y, dirs = 8):
    ...
    delta_x = np.diff(x)
    delta_y = np.diff(y)
    thetas = np.arctan2(delta_y, delta_x) #angles
...
categories = ['aa', 'alif', 'alif mad aa', 'ayn', 'baa', 'bari yaa', 'cheey', 'chey', 'choti yaa', 'daal', 'dhaal', 'faa', 'gaaf', 'ghain', 'haa1', 'haa2', 'haa3', 'hamza', 'jeem', 'kaaf', 'khaa', 'laam', 'meem', 'noon', 'noonghunna', 'paa', 'qaaf', 'raa', 'rhraa', 'seen', 'seey', 'sey', 'sheen', 'swaad', 'taa', 'ttaa', 'twa', 'twaa', 'waw', 'yaa', 'zaaa', 'zaal', 'zhaa', 'zwaa', 'zwaad']
clusters = {}
clusters['alif'] = ['alif']
clusters['alif mad aa'] = ['alif mad aa', 'aa']
clusters['bey'] = ['ttaa', 'paa', 'seey', 'baa', 'taa', 'sey']
clusters['jeem'] = ['khaa', 'jeem', 'haa1', 'cheey', 'chey']
clusters['daal'] = ['daal', 'zaal', 'dhaal']
clusters['ray'] = ['rhraa', 'raa', 'zhaa', 'zaaa']
clusters['seen'] = ['seen', 'sheen']
clusters['swad'] = ['zwaad', 'swaad']
clusters['twa'] = ['twaa', 'zwaa', 'twa']
...
clusters['choti-yay'] = ['choti yaa', 'yaa']
clusters['bari-yay'] = ['bari yaa']
clusters['hamza'] = ['hamza']

cluster_keys = sorted(clusters.keys())
char_to_cluster = {}
for key in clusters:
  for val in clusters[key]:
    cat = categories.index(val)
    char_to_cluster[cat] = cluster_keys.index(key)
...
def get_char_score(data, char):
    dirs = 8
    whole_x, whole_y, penup, img = get_whole_stroke(data)
    height = 200
    ...
    for i in p:
        x = scaled_x[start: i]
        y = scaled_y[start: i]
        x_y = rdp(list(zip(x, y)), epsilon=1.5)
        new_scaled_x += [j[0] for j in x_y]
        new_scaled_y += [j[1] for j in x_y]
        codes += (get_chain_codes(new_scaled_x, new_scaled_y, dirs=8))
    ...
    bins = np.zeros(dirs)
    for i in range(dirs):
        bins[i] = np.sum(bins == i) / len(codes)
    ...
    cat = categories.index(char)

    mean_x = np.load(os.path.join(__location__, 'mean_x.npy'))
    std_x = np.load(os.path.join(__location__, 'std_x.npy'))
    mean_y = np.load(os.path.join(__location__, 'mean_y.npy'))
    X = np.zeros((1, 49 + 48 + dirs))
    X[0] = np.concatenate([hbr, kaligo, bins])

    X = np.nan_to_num((np.nan_to_num(X) - mean_x) / std_x)
    clust = char_to_cluster[cat]
    model_name = os.path.join(__location__, f'URDU_character_regresion_{clust}.sav')
    model = joblib.load(model_name)
    Y = model.predict(X) + mean_y
